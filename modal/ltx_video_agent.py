...
# Deploy this file to replace the existing mvs-ltx-video app. Keeping the same
# app and endpoint names preserves MODAL_LTX_URL in Render.
...
class LTXGenerator:
    ...
    def load_model(self) -> None:
        ...
        print(f"[LTX-2.3 Agent] Loading {MODEL_ID}...")
        ...
        model_volume.commit()
        print("[LTX-2.3 Agent] Model ready.")
    ...
    def generate_clip(
        ...
        try:
            ...
            if character_required and not init_image_url:
                raise ValueError(
                    "Character conditioning is required. Generation was blocked because no character image was attached."
                )
            ...
            conditions = None
            if init_image_url:
                condition_type = "required character" if character_required else "visual"
                print(
                    f"[LTX-2.3 Agent] Loading {condition_type} first-frame condition for job "
                    f"{job_id or 'untracked'}: {init_image_url}"
                )
            ...
            elif character_required:
                # Keep the invariant close to the model call too. The endpoint
                # and Render API both enforce it, but the GPU method must never
                # silently accept a direct invocation without a condition.
                raise ValueError("required character condition is missing")

            print(
                f"[LTX-2.3 Agent] Generating {num_frames} frames at {fps} fps "
                f"for job {job_id or 'untracked'}; character_required={character_required}; "
                f"conditioned={conditions is not None}"
            )
            ...
            result = {
                ...
                "job_id": job_id,
                "video_url": video_url,
                "character_conditioned": bool(character_required and conditions),
            }
            self._send_webhook(webhook_url, result)
            print(f"[LTX-2.3 Agent] Completed job {job_id}: {video_url}")
            return result
        except Exception as error:
            message = f"{type(error).__name__}: {error}"
            print(f"[LTX-2.3 Agent] Job {job_id} failed: {message}")
            self._send_webhook(
                webhook_url,
...
def generate(payload: dict):
    ...
    character_required = bool(payload.get("character_required", False))
    if character_required and not init_image_url:
        raise HTTPException(
            status_code=400,
            detail="Character conditioning is required. Generation was not started because no character image was attached.",
        )
    ...
    return {
        ...
        "character_required": character_required,
        "conditioned": bool(init_image_url),
    }
